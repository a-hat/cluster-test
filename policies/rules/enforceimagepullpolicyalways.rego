...
hasValidImagePullPolicyField(container) {
...
isPartOfMesh(containersType, container) {
	...
	not image_not_matching(container.image, "gcr.io/linkerd-io/proxy")
}

else {
	containersType == "initContainers"
	container.name == "linkerd-init"
	not image_not_matching(container.image, "gcr.io/linkerd-io/proxy-init")
}

else {
	containersType == "containers"
	container.name == "istio-operator"
	not image_not_matching(container.image, "docker.io/istio/operator")
}

else {
	containersType == "initContainers"
	container.name == "istio-init"
	not image_not_matching(container.image, "docker.io/istio/proxyv2")
}
...
rule[reason] {
	thereIsNoExclusionAnnotationForConstraint("enforceimagepullpolicyalways")
	container := metadata.objectSpec[container_type][_]

	container_types := {"containers", "initContainers"}
	container_types[container_type]

	not isPartOfMesh(container_type, container)
	not hasValidImagePullPolicyField(container)
	reason := printReasonWithObject("EnforceImagePullPolicyAlways", "usage of pre-pulled images is not allowed", "", "imagePullPolicy", container.name, container_type, "'Always'")
}
