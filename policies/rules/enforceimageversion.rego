...
rule[reason] {
	thereIsNoExclusionAnnotationForConstraint("enforceimageversion")
	container := podSpec[container_type][_]
	container_types := {"containers", "initContainers"}
	container_types[container_type]

	not contains(container.image, ":")
	reason := printReasonWithObject("EnforceImageVersion", "No explicit image version for the container", "", "image", container.name, container_type, "")
}
...
rule[reason] {
	...
	reason := printReasonWithObject("EnforceImageVersion", "Floating 'latest' tag used for the container image", "", "image", container.name, container_type, "specific version tag")
}
