...
trustedimages = {repo | repo := inputParams.trustedimages[_]} {
...
imageAllowedNamespace(image) {
	...
	any([check |
		...
		not image_not_allowed_agg(image, it)
		check := true
	])
}
...
registry_allowed_slash(image, reg) {
	not reg == "/"
	endswith(reg, "/")
	startswith(image, reg)
}

registry_allowed_slash_missing(image, reg) {
	not reg == ""
	not endswith(reg, "/")
	startswith(image, sprintf("%v/", [reg]))
}

registry_not_allowed_agg(image, reg) {
	not registry_allowed_slash(image, reg)
...
registryAllowedNamespace(image) {
	...
	any([check |
		...
		not registry_not_allowed_agg(image, it)
		check := true
	])
}
...
rule[reason] {
	trace(concat("", ["INPUT_OBJECT: ", json.marshal(inputObject)]))
	trace(concat("", ["TRUSTED_REGISTRIES: ", json.marshal(trustedregistries)]))
	trace(concat("", ["TRUSTED_IMAGES: ", json.marshal(trustedimages)]))

	thereIsNoExclusionAnnotationForConstraint("imageallowlist")

	container := podSpec[container_type][_]

	container_types := {"containers", "initContainers"}
	container_types[container_type]

	registryAllowed := any([check |
		it := trustedregistries[_]
		not registry_not_allowed_agg(container.image, it)
		check := true
	])

	imageAllowed := any([check |
		it := trustedimages[_]
		not image_not_allowed_agg(container.image, it)
		check := true
	])
	...
	not imageAllowedNamespace(container.image)

	reason := printReasonWithObject("ImageAllowlist", sprintf("The image '%v' for the container is not on the allowlist", [container.image]), "", "image", container.name, container_type, "")
}
