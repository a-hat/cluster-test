...
hasCPURequest(container) {
	has_field(container, "resources")
	has_field(container.resources, "requests")
	has_field(container.resources.requests, "cpu")
	not is_null(container.resources.requests.cpu)
}
...
rule[reason] {
	thereIsNoExclusionAnnotationForConstraint("enforcecpurequest")
	container := podSpec[container_type][_]

	container_types := {"containers", "initContainers"}
	container_types[container_type]

	not hasCPURequest(container)
	reason := printReasonWithObject("EnforceCPURequest", "CPU request must be specified", "", "resources.requests.cpu", container.name, container_type, "")
}

CPUValueIsValid(value) {
	memoryValueRegex := "^((0\\.[0-9]+)|([0-9]{2,})|[1-9])[m]?$"
	re_match(memoryValueRegex, value)
}
...
rule[reason] {
	...
	not CPUValueIsValid(container.resources.requests.cpu)
	reason := printReasonWithObject("EnforceCPURequest", "CPU request is invalid", "", "resources.requests.cpu", container.name, container_type, "")
}
